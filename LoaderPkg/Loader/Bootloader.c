...
UefiMain(
  ...
  while (!Connected) {
    ...
    for (UINTN i = 0; i < xsdt->Header.Length; i++) {
        sum += ((UINT8 *)&(xsdt->Header))[i];
    }
    if (sum)
        DEBUG((DEBUG_ERROR, "Problem with checksum in XSTD\n"));

    UINTN rsdt_len = (xsdt->Header.Length - sizeof(XSDT)) / 8;
    EFI_ACPI_DESCRIPTION_HEADER *head = NULL;
    EFI_ACPI_6_2_FIXED_ACPI_DESCRIPTION_TABLE *fadt = NULL;
    for (UINTN i = 0; i < rsdt_len; i++) {
        head = (EFI_ACPI_DESCRIPTION_HEADER *)xsdt->Tables[i];
        if (head->Signature == EFI_ACPI_6_2_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE) {
            DEBUG((DEBUG_INFO, "FACP found!\n"));
            fadt = (EFI_ACPI_6_2_FIXED_ACPI_DESCRIPTION_TABLE *)head;
        }
    }
    UINTN dsdt_phys = fadt->XDsdt;
